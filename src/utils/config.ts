...
const devDefaults = {
  ...
  PHOENIXSTORE_API_URL: 'http://localhost',
  PHOENIXSTORE_PORT: '3000',
...
const validateConfig = () => {
  const missingVars = [];
  const environment = process.env.PHOENIXSTORE_ENV || 'development';
  ...
  if (!process.env.PHOENIXSTORE_PORT) missingVars.push('PHOENIXSTORE_PORT');
  ...
  if (missingVars.length > 0) {
    ...
    missingVars.forEach(variable => {
      console.warn(`    - ${variable}: using default value '${(devDefaults as any)[variable]}'`);
    });
    
    if (environment === 'production') {
      console.warn('\n[!!] IMPORTANT: Using development defaults in production is not recommended!');
      console.warn('     Please set all environment variables in your production environment.\n');
    } else {
      console.warn('\n[!] Note: Development defaults will be used for missing variables.');
      console.warn('    This is OK for development but ensure all variables are set for production.\n');
    }
  } else {
    console.log('\n[*] Environment Variables Status:');
    console.log('--------------------------------');
